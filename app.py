...
def video_attendance():
    ...
    while cap.isOpened():
        ...
        if pred_label not in recognized_students:
            ...
            row = c.fetchone()
            if row:
                # Check one-hour gap before inserting record
                if can_record_attendance(pred_label):
                    recognized_students[pred_label] = row[0]
                    ts = datetime.datetime.utcnow().isoformat()
                    c.execute("INSERT INTO attendance (student_id, name, timestamp) VALUES (?, ?, ?)", (pred_label, row[0], ts))
                    conn.commit()
    conn.close()
    cap.release()
    os.remove(video_path)
...
@app.route("/realtime_knn", methods=["GET"])
def realtime_knn():
    """
    Run realtime KNN-based face recognition (OpenCV + scikit-learn).
    Attendance is logged into the same database.
    """
    video = cv2.VideoCapture(0)
    facedetect = cv2.CascadeClassifier('data/haarcascade_frontalface_default.xml')

    # Load labels and faces dataset
    try:
        ...
        return jsonify({"error": f"Could not load face dataset: {e}"}), 500
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            c.execute("INSERT INTO attendance (student_id, name, timestamp) VALUES (?, ?, ?)", 
                      (0, student_name, ts))   # student_id=0 since names.pkl may not map to DB IDs
            conn.commit()
            conn.close()

            recognized_students.add(student_name)

        cv2.imshow("Realtime KNN Attendance", frame)
    ...
    return jsonify({"recognized": list(recognized_students)})
...
@app.route("/midday_meal")
def midday_meal():
    # Get today's attendance and compute
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    pres_students = c.fetchone()[0]

    absent_students = total_students - pres_students
    meals_served = pres_students

    # Sample calculations (1 serving = 60g Dal, 100g Rice for simplicity)
    dal_needed_kg = round(pres_students * 0.06, 2)  # 60g per student
    rice_needed_kg = round(pres_students * 0.1, 2)  # 100g per student

    perc_present = round( (pres_students / total_students)*100 if total_students else 0, 1)
    ...
    return render_template("midday_meal.html",
        ...
        midday_meals_served = meals_served,
        perc_present = perc_present,
